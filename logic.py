...
class DatabaseManager:
    ...
    def get_winners_count(self, prize_id):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute('SELECT COUNT(*) FROM winners WHERE prize-id = ?', (prize_id, ))
        return cur.fetchall()[0][0]
    ...
    def get_rating(self):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute('''
SELECT user.user_name, COUNT(winners.prize_id) AS count_prizes
FROM winners                        
INNER JOIN users ON user.user_name = winners.user_name
GROUP BY winners.user_id                        
ORDER BY count_prizes DESK
LIMIT 10                        
''')
        return cur.fetchall()
